...
@synthesize delegate, prizeType, isVisible, globalDelegate, type, notificationPosition, wrapper;
...
- (id)initWithContent:(BGiveBedollarsWrapper *)gbWrapper
{
    if (self = [super init])
    {
        [self initElements];
...
{    
    ...
    NSString *content = wrapper.content;
...
{
    CGRect startingFrame = [Beintoo getAppWindow].frame;
...
- (void)openTemplateControllerWithURL:(NSString *)URL
{
    ...
    [Beintoo giveBedollars:self launchControllerWithURL:URL];
}
...
- (void)closeTemplate
{
    ...
    [Beintoo hideGiveBedollars:self];
}
...
- (void)keyboardWillShow:(NSNotification *)notification;
{
    ...
    if (self.notificationPosition == BeintooNotificationPositionBottom && !isUpForKeyboard)
    {
        if ([Beintoo appOrientation] == UIInterfaceOrientationLandscapeLeft || [Beintoo appOrientation] == UIInterfaceOrientationLandscapeRight) {
            ...
            [UIView animateWithDuration:0.25
                             animations:^(void){
                                 self.frame = CGRectOffset(self.frame, 0, - offset); // CGRectMake(self.frame.origin.x, self.frame.origin.y - offset, self.frame.size.width, self.frame.size.height);
                             }
                             completion:^(BOOL finished){
                                 isUpForKeyboard = YES;
                                 
                                 CGRect rect = recommWebView.frame;
                                 recommWebView.scrollView.bounds = rect;
                             }
             ];
        }
    }
